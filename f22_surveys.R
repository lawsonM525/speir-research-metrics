...
# Load the data
pre_survey <- read.csv("csv-files/F22_Pre_Survey_Cleaned.csv")
post_survey <- read.csv("csv-files/F22_Post_Survey_Cleaned.csv")
pre_survey_num <- read.csv("csv-files/F22_Pre_Survey_Numeric.csv")
post_survey_num <-read.csv("csv-files/F22_Post_Survey_Numeric.csv")
...
plot_ans <- function(data, column, df_name, scale_type) {
  ...
  for (section in unique(data$section)) {
    ...
    ggsave(
      filename = 
        paste0("survey-plots/f22/", df_name, "_", column, "_section_", section, ".png"),
...
for (q_col in question_cols) {
  ...
  df_s3 <- data.frame(s3 = pre_survey_num[pre_survey_num$section == 3, q_col])
  
  # Calculate the maximum number of rows among all sections
  max_rows <- max(nrow(df_s1), nrow(df_s2), nrow(df_s3))
  
  # Pad the vectors with NA's to match the maximum length
  df_s1 <- rbind(df_s1, data.frame(s1 = rep(NA, max_rows - nrow(df_s1))))
  df_s2 <- rbind(df_s2, data.frame(s2 = rep(NA, max_rows - nrow(df_s2))))
  df_s3 <- rbind(df_s3, data.frame(s3 = rep(NA, max_rows - nrow(df_s3))))
  
  # Check if df_s1, df_s2, and df_s3 have rows before proceeding
  if (nrow(df_s1) > 0 && nrow(df_s2) > 0 && nrow(df_s3) > 0) {
    # Combine all dataframes into one with separate columns for each section
    df_combined <- data.frame(s1 = df_s1$s1, s2 = df_s2$s2, s3 = df_s3$s3)
    
    # Call compute_stats function
    results <- compute_stats(df_combined, c("s1", "s2", "s3"), paste0("pre-survey/", q_col))
